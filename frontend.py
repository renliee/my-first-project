...
st.set_page_config(page_title="ChatBot Restoran", page_icon="🤖")
...
def parse_order(text):
    ...
    for key, item in MENU.items():
        if key in text:
            ...
            pattern1 = r'(\d+)\s*(?:x|buah|porsi)?\s*' + re.escape(key) #handle 2x sate ayam
            pattern2 = re.escape(key) + r'\s*(\d+)' #handle saye ayam 2 porsi (qty after)
...
with st.sidebar:
    if "orders" in st.session_state and st.session_state.orders: #if there is orders
        ...
        total = 0
        for key, item in st.session_state.orders.items(): #will always count and write from 0 again, but's fine bcs all items was saved at session_state.orders
            subtotal = item["price"] * item["qty"]
        ...
        with col1: #the code of col1 button
            if st.button("Checkout", type="primary", use_container_width=True, disabled=checkout_disabled): #button disabled (cant click) if checkout_disabled == True
                ...
                if st.button("Kirim", type = "primary", use_container_width = True): #if user clicked "Kirim"
                    ...
                    try:
                        ...
                        response = requests.post( #response contains all info: response.text, .status_code, .url, etc.
                            f"{FASTAPI_URL}/orders", 
                            json={ #must be the same as the schemas input at backend and variable name must be the same as the schemas variable name
                                "customer_name": "Guest", 
    ...
    if st.session_state.last_receipt: #out of the indent bcs orders will reset to 0 after checkout, so user still can see the status of orders after checkout
        order_id = st.session_state.last_receipt['id']
        try:
            status_response = requests.get(f"{FASTAPI_URL}/orders/{order_id}")
            if status_response.status_code == 200:
                ...
                if status == "pending":
                    st.warning(f"⏳ Order {order_id:03d}: Menunggu diproses")
                elif status == "processing":
                    st.info(f"🍳 Order {order_id:03d}: Sedang dimasak")
                elif status == "completed":
                    st.success(f"✅ Order {order_id:03d}: Siap diantar!")
